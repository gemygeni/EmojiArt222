...
class EmojiArtViewController: UIViewController, UIDropInteractionDelegate, UIScrollViewDelegate, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, UICollectionViewDragDelegate, UICollectionViewDropDelegate {
    ...
    var emojiArt : EmojiArt?{
        ...
        set {
            ...
            if let url = newValue?.url {
                _ = ImageFetcher(fetch: url){ (url , image) in
                    DispatchQueue.main.async {
                        self.emojiArtBackgroundImage = (url, image)
                        newValue?.emojis.forEach{
                            let attributedstring = $0.text.attributedString(withTextStyle: .body, ofSize: CGFloat($0.size))
                            self.emojiArtView.addLabel(with: attributedstring, centerdAt: CGPoint(x: $0.x, y: $0.y ))
                        }
                    }
                }
            }
    ...
    @IBAction func save(_ sender: UIBarButtonItem) {
      if let json = emojiArt?.json{
        ...
        in: .userDomainMask,
         appropriateFor: nil,
        create: true
         ).appendingPathComponent("hhhhhhhhhh.json") {
    ...
    override func viewWillAppear(_ animated: Bool) {
        if let url = try? FileManager.default.url(for: .documentDirectory,
                                                  in: .userDomainMask,
                                                  appropriateFor: nil,
                                                  create: true).appendingPathComponent("hhhhhhhhhh.json"){
    ...
    var _emojiArtBackgroundURL : URL?
